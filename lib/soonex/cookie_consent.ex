defmodule Soonex.CookieConsent do
  @moduledoc false

  use Phoenix.Component
  use Corex

  @dialog_id "cookie-consent-dialog"
  @banner_id "cookie-consent-banner"
  @storage_key "soonex:cookie-consent"

  def dialog_id, do: @dialog_id
  def banner_id, do: @banner_id
  ...
  def head_script do
    ...
    <script>
      (() => {
        try {
          const raw = localStorage.getItem(#{Jason.encode!(@storage_key)});
          if (!raw) return;
          const parsed = JSON.parse(raw);
          if (!parsed || typeof parsed !== "object") return;
          document.documentElement.dataset.cookieConsent = "set";
          document.documentElement.dataset.cookieAnalytics = parsed.analytics ? "1" : "0";
          document.documentElement.dataset.cookieMarketing = parsed.marketing ? "1" : "0";
        } catch (_e) {}
      })();
    </script>
    """)
  ...
  attr(:privacy_path, :string, required: true)

  def cookie_consent(assigns) do
    assigns = assign(assigns, :banner_id, @banner_id)

    ~H"""
    <div
      id={@banner_id}
      class="pointer-events-none fixed inset-x-0 bottom-0 z-40 p-space-sm"
      hidden
      data-cookie-consent-banner
      role="dialog"
      aria-modal="false"
      aria-labelledby="cookie-consent-banner-title"
      aria-describedby="cookie-consent-banner-desc"
    >
      <div class="pointer-events-auto mx-auto flex w-full max-w-xl flex-col gap-space-sm rounded-lg border border-border bg-surface p-space-sm shadow-md">
        <div class="flex min-w-0 flex-col gap-space-xs">
          <h2 id="cookie-consent-banner-title" class="m-0 text-lg font-semibold leading-lg text-ink">
            <span class="inline-flex min-w-0 items-center gap-space-xs">
              <.cookie_icon class="size-[1.25em] shrink-0" />
              <span>Cookie preferences</span>
            </span>
          </h2>
          <p id="cookie-consent-banner-desc" class="m-0 text-pretty text-base text-ink-muted">
            Necessary preferences stay on your device. Analytics and marketing stay off unless you allow them.
          </p>
        </div>

        <div class="flex min-h-size-sm flex-wrap items-center justify-between gap-x-space-sm gap-y-space-xs max-sm:flex-col max-sm:items-stretch">
          <.navigate
            to={@privacy_path}
            class="link ui-nav ui-size-sm ui-width-fit shrink-0 self-center text-ink-muted max-sm:self-start"
          >
            Privacy
          </.navigate>
          <div class="ms-auto flex flex-wrap items-center justify-end gap-space-xs max-sm:ms-0 max-sm:w-full">
            <.action
              type="button"
              class="button ui-ghost ui-size-sm ui-rounded-md ui-width-fit"
            ...
            <.action
              type="button"
              class="button ui-brand ui-solid ui-size-sm ui-rounded-md ui-width-fit text-brand-contrast"
  ...
  def cookie_dialog(assigns) do
    assigns = assign(assigns, :dialog_id, @dialog_id)

    ~H"""
    <.dialog
      id={@dialog_id}
      class="dialog cookie-consent-dialog ui-size-md ui-rounded-lg ui-width-fit ui-max-height-dvh [&_[data-part=positioner]]:p-space-md [&_[data-part=description]]:text-ink-muted"
      modal
      prevent_scroll
      animation="instant"
    >
      <:trigger class="link ui-nav ui-size-sm text-ink-muted">
        Cookies
      </:trigger>
      <:title>
        <span class="inline-flex min-w-0 items-center gap-space-sm">
          <.cookie_icon class="size-[1.25em] shrink-0" />
          <span>Cookie preferences</span>
        </span>
      </:title>
      <:description>
        Necessary preferences always stay on. Toggle the rest below.
      </:description>
      <:close_trigger>
        <.heroicon name="hero-x-mark" />
      </:close_trigger>
      <:content class="flex min-h-0 max-h-dvh w-full flex-col overflow-hidden ui-width-full sm:ui-width-xl">
        <div class="min-h-0 flex-1 overflow-y-auto py-space-sm sm:py-space scrollbar scrollbar--sm">
          <ul class="m-0 flex w-full list-none flex-col p-0">
            <li class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-space-sm border-t border-border py-space-sm first:border-t-0 first:pt-0">
              <div class="flex min-w-0 flex-col gap-space-xs">
                <p class="m-0 text-base font-semibold leading-normal text-ink">Necessary</p>
                <p class="m-0 text-pretty text-sm text-ink-muted">
                  Theme, mode, and accessibility settings required for the site to work.
                </p>
              </div>
              <.switch id="cookie-consent-necessary" class="switch ui-size-sm" checked disabled>
                <:label class="sr-only">Necessary cookies</:label>
              </.switch>
            </li>
            <li class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-space-sm border-t border-border py-space-sm">
              <div class="flex min-w-0 flex-col gap-space-xs">
                <p class="m-0 text-base font-semibold leading-normal text-ink">Analytics</p>
                <p class="m-0 text-pretty text-sm text-ink-muted">
                  Help us understand how the site is used. Off by default on this marketing site today.
                </p>
              </div>
              <.switch id="cookie-consent-analytics" class="switch ui-size-sm" name="analytics">
                <:label class="sr-only">Analytics cookies</:label>
              </.switch>
            </li>
            <li class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-space-sm border-t border-border py-space-sm">
              <div class="flex min-w-0 flex-col gap-space-xs">
                <p class="m-0 text-base font-semibold leading-normal text-ink">Marketing</p>
                <p class="m-0 text-pretty text-sm text-ink-muted">
                  Optional marketing tags. Off by default; nothing loads unless you allow them.
                </p>
              </div>
              <.switch id="cookie-consent-marketing" class="switch ui-size-sm" name="marketing">
                <:label class="sr-only">Marketing cookies</:label>
              </.switch>
            </li>
          </ul>
        </div>

        <div class="mt-space-sm flex shrink-0 flex-wrap justify-end gap-space-xs border-t border-border pt-space-sm">
          <.action
            type="button"
            class="button ui-ghost ui-size-sm ui-rounded-md ui-width-fit"
            data-cookie-consent="reset"
          >
            Essential only
          </.action>
          <.action
