...
permalink: /docs/
description: Clone corex-ui repos, run Mix and Tableau, tune themes, plus a Markdown gallery (tables, alerts, footnotes, images) matching MDEx in config.
tags:
  - Elixir
  - Tableau
  - Corex
...
If you develop from the monorepo checkout, use `templates/soonex`. The default is **`{:corex, "~> 0.1.0"}`** from Hex; to build against a sibling **`corex`** repo, switch **`mix.exs`** to **`{:corex, path: "../../corex"}`** and run **`mix deps.get`** so **`deps/corex`** matches that checkout (the **`corex/hooks`** and **`corex/select`** style imports in **`assets/js/site.js`** stay the same).
...
- Directory renames such as `lib/soonex/` → `lib/acme/`, plus `lib/soonex_web/` → `lib/acme_web/` when that directory exists, and matching `test/` trees when present
...
1. Edit `config/config.exs` for Tableau and asset tooling.
...
> **Tip:** After `mix tableau.build`, open `_site/docs/index.html` to confirm exactly what shipped for this permalink.
